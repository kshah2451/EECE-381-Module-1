...
extern unsigned int *audio_buffer_second;
...
void mainGame_level2(alt_up_ps2_dev *ps2_kb, KB_CODE_TYPE decode_mode, alt_u8 data, char ascii){
	...
	if(game_data != NULL){
		...
		set_jellyfish();
		heads_up_display_static(pixel_buffer, char_buffer);
		...
		{
			display_resources();
			if(game_data->towers[grid_pos]->isAlive == 1){
				display_health(game_data->towers[grid_pos]->health);
			}
			else if(game_data->towers[grid_pos]->isAlive == 0){
				display_health(0);
			}
			//play_loop(audio_buffer_second,audio,2);
			if (decode_scancode(ps2_kb, &decode_mode, &data, &ascii)==0)
			{
					...
					else if( data == FOUR_KEY || data == FIVE_KEY || data == SIX_KEY || data == SEVEN_KEY || data == EIGHT_KEY) {
						tower_selection(ps2_kb, decode_mode, data, ascii, temp_baby_attributes);
		...
		free(game_data);
		free(audio_buffer_second);
	}
	else{
		printf("ERROR in level 2 game data malloc \n");
	}
