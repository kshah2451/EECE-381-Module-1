	...
	while(1)
	 {
		...
		if (decode_scancode(ps2_kb, &decode_mode, &data, &ascii)==0)
		{
				...
				if(data == 0x16 || data == 0x1E || data == 0x26){
					...
					tower_selection(baby, grid_pos, ps2_kb, decode_mode, data, ascii, temp_baby_attributes);
				...
				if(data == 0x1c && towerCanBePlaced == 1 && (baby[grid_pos].isAlive == 0)){ // user presses A
					set_baby_attributes(baby, grid_pos, temp_baby_attributes);
					draw_baby(baby, grid_pos, pixel_buffer);
					baby[grid_pos].isAlive = 1;
				...
				if(data == 0x32 && (baby[grid_pos].isAlive == 1)){

					remove_baby(baby, grid_pos, pixel_buffer);
				}
